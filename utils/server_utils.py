"""CARLA server lifecycle: start/stop instances, kill running processes."""
import subprocess
import os
import time
    ...
    def start(self):
        ...
        for cfg in self.env_configs:
            ...
            cmd = (
                ...
                f'xvfb-run -s "-screen 0 640x480x24" '
                f'{self._carla_sh_str} '
                f'-carla-server -noRendering -quality-level=Low -fps=10 -carla-rpc-port={cfg["port"]}'
            )CUDA_VISIBLE_DEVICES
            """
            cmd = f'SDL_HINT_CUDA_DEVICE=0 bash {self._carla_sh_str} ' \
                  f'-fps=10 -carla-rpc-port={cfg["port"]} -RenderOffScreen -nullrhi -quality-level=Epic -nosound'
