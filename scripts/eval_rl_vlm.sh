...
# * To eval for the NoCrash benchmark  80
eval_ () {
  python -u eval_rl_vlm.py resume=true log_video=false save_to_wandb=true \
  agent.rl_vlm.wb_run_path=null \
  wb_project=found_rl wb_name=drqv2_rClip_lb_test\
  test_suites=lb_test \
  n_episodes=160 \
