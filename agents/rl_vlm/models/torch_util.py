...
def convert_jump_gap_to_actions(jump_gap_list):
    has_vlm_actions = []

    # If first gap != 0, there are gap-1 zeros before first action
    if jump_gap_list and jump_gap_list[0] > 1:
        has_vlm_actions.extend([0] * (jump_gap_list[0] - 1))

    has_vlm_actions.append(1)  # First action

    for gap in jump_gap_list[1:]:
        has_vlm_actions.extend([0] * (gap - 1))
        has_vlm_actions.append(1)

    return has_vlm_actions
