...
from agents.rl_vlm.models.distributions import BetaDistribution
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        # Action distribution
        self.action_dist = make_proba_distribution(action_space, use_sde=use_sde, dist_kwargs=dist_kwargs, use_beta=self.use_beta)
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            self.dist_mu, self.dist_sigma = self.action_dist.proba_distribution_net(latent_dim=latent_dim_pi)
        else:
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False, clip_action: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        if isinstance(self.action_dist, BetaDistribution):
            distribution, mu, sigma = self._get_action_dist_from_latent(latent_pi)
        else:
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        ...
        if isinstance(self.action_dist, DiagGaussianDistribution):
            ...
            return self.action_dist.proba_distribution(mu,
                                                       sigma), mu.detach().cpu().numpy(), sigma.detach().cpu().numpy()
        else:
    ...
    def _predict(self, observation: PyTorchObs, deterministic: bool = False) -> th.Tensor:
        ...
        # If BetaDistribution (or any with low/high), map back to env action_space
        d_low, d_high = self.action_dist.low, self.action_dist.high

        if d_low is not None and d_high is not None:
            # Torch unscale, same as np version
            a_low = th.as_tensor(self.action_space.low, device=actions.device, dtype=actions.dtype)
            a_high = th.as_tensor(self.action_space.high, device=actions.device, dtype=actions.dtype)
            actions = (actions - d_low) / (d_high - d_low) * (a_high - a_low) + a_low
            actions = th.clamp(actions, a_low, a_high)

        return actions

    def evaluate_actions(self, obs: PyTorchObs, actions: th.Tensor, exploration_suggests: Optional[th.Tensor]=None) -> tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        ...
        if exploration_suggests is not None:
            exploration_loss = distribution.exploration_loss(exploration_suggests)
            return values, log_prob, entropy, exploration_loss
        else:
            return values, log_prob, entropy, 0.
