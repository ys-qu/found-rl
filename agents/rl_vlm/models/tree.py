...
class SumTree:
    ...
    def get(self, cumsum):
        ...
        if np.isnan(self.total) or np.isinf(self.total) or self.total <= 0:
        ...
        # Check if the data slot is initialized
        if self.data[data_idx] is None:
            # Find the next valid data slot
            for i in range(1, self.size):
                check_idx = (data_idx + i) % self.size
                if self.data[check_idx] is not None:
                    data_idx = check_idx
                    break
            else:
                # If no valid slot found, return None to indicate failure
                return None, None, None
