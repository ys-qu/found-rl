...
from agents.rl_vlm.models.actor_critic_policy import ActorCriticCnnPolicy, ActorCriticPolicy, BasePolicy, MultiInputActorCriticPolicy
...
class PPOVlm(PPO):
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                values, log_prob, entropy, exploration_loss = self.policy.evaluate_actions(rollout_data.observations, actions,
                ...
                # Optional imitation loss
                if self.use_vlm:
                    actions_pi, _, _ = self.policy.forward(rollout_data.observations, deterministic=False)
                    # print('self.buffer.vlm_pos: ', self.rollout_buffer.vlm_pos)
                    # print('self.buffer.pos: ', self.rollout_buffer.pos)
                    if sum(rollout_data.has_vlm_actions) > 0:
                        imitation_loss = F.mse_loss(actions_pi[rollout_data.has_vlm_actions],
                                                    rollout_data.vlm_actions[rollout_data.has_vlm_actions])
                    else:
                        imitation_loss = th.zeros_like(policy_loss)
                else:
                    imitation_loss = th.zeros_like(policy_loss)
                ...
                loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss  + \
                       self.explore_coef * exploration_loss + 2 * imitation_loss
                #######################################################################################################self.imitation_coef
        ...
        if hasattr(self.policy, "log_std"):
            self.train_debug.update({"train/std": th.exp(self.policy.log_std).mean().item()})
        if self.clip_range_vf is not None:
            self.train_debug.update({"train/clip_range_vf": clip_range_vf})
        if self.use_vlm:
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            #     e.update_num_timesteps(self.num_timesteps)
            t0 = time.time()
