...
class ValeoAction(object):
    ...
    def __init__(self, ego_vehicle, neglect_rl_stop=True):
    ...
    def get(self, terminal_reward):
        ...
        if self._neglect_rl_stop:
            desired_speed = min(self._maxium_speed, desired_spd_veh, desired_spd_ped)
        else:
        ...
        # Green light acceleration reward to encourage proceeding
        r_acc = 0.0
        # if not self._neglect_rl_stop:
        #     if light_state == carla.TrafficLightState.Green and desired_speed > 0.1:
        #         ev_acc = self._ego_vehicle.vehicle.get_acceleration()
        #         ev_acc_norm = np.linalg.norm(np.array([ev_acc.x, ev_acc.y]))
                
        #         # Reward for positive acceleration (moving forward)
        #         if ev_acc_norm > 0.1:  # Threshold to avoid noise
        #             # Reward based on acceleration magnitude, capped at reasonable value
        #             r_acc = min(0.5, ev_acc_norm * 0.1)  # Scale factor 0.2, max reward 0.5
        #         else:
        #             # Small penalty for staying still at green light
        #             r_acc = -0.1
            
        # Enhanced green light proceed reward
        r_gl_proceed = 0.0
        # if not self._neglect_rl_stop:
        #     if light_state == carla.TrafficLightState.Green and desired_speed > 0.1:
        #         if ev_speed > 0.1:  # Any movement
        #             r_gl_proceed = 0.5 * (ev_speed / self._maxium_speed)
        #         else:
        #             r_gl_proceed = 0.0
        """
        When training sac, we found the agent can hardly proceed in green light
        """
        # cost_green_stop = 0
        # if light_state == carla.TrafficLightState.Green:
        #     dist_gl = max(0.0, np.linalg.norm(light_loc[0:2]) - 5.0)
        #     if dist_gl < 5.0 and desired_speed > 0.0:
        #         cost_green_stop = - np.abs(ev_speed-desired_speed) / self._maxium_speed
        ...
        # r_progress - reward for route completion progress in meters
        r_progress = 0.0
        # if not hasattr(self, '_last_route_completed'):
        #     self._last_route_completed = self._ego_vehicle._route_completed
        
        # current_route_completed = self._ego_vehicle._route_completed
        # route_progress_meters = max(current_route_completed - self._last_route_completed, 0.)
        
        # # Normalize by max possible travel per step: 6 * 1.5 m/s / 10 fps = 0.9 m/step
        # max_travel_per_step = self._maxium_speed / 10.0
        # if desired_speed > 0.1:
        #     r_progress = np.clip(route_progress_meters / max_travel_per_step, 0.0, 1.0)
        # else:
        #     r_progress = 1 - np.clip(route_progress_meters / max_travel_per_step, 0.0, 1.0)
        
        # self._last_route_completed = current_route_completed
        

        # if not self._neglect_rl_stop:
        #     if light_state == carla.TrafficLightState.Green:
        #         r_position = 0.5 * r_position
        #         r_rotation = 0.5 * r_rotation
        #         r_action = 0.

        reward = r_speed + r_position + r_rotation + terminal_reward + r_action  # + r_acc + r_gl_proceed + r_progress  # * 0.5
        ...
        debug_texts = [
            ...
            f'ev_spd:{ev_speed:5.2f} r_pr:{r_progress:5.2f}',
            ...
            # f'r_acc:{r_acc:5.2f} r_gl:{r_gl_proceed:5.2f}'
        ]

        reward_debug = {
            'debug_texts': debug_texts
        }
        reward_debug['r_position'] = r_position
        reward_debug['r_rotation'] = r_rotation
        reward_debug['terminal_reward'] = terminal_reward
        reward_debug['r_action'] = r_action
        reward_debug['ev_speed'] = ev_speed
        reward_debug['desired_speed'] = desired_speed
        reward_debug['maxium_speed'] = self._maxium_speed
        reward_debug['cost'] = - r_position - r_rotation - terminal_reward
