...
class TrafficLightHandler:
    ...
    @staticmethod
    def get_light_state_with_timing(vehicle, offset=0.0, dist_threshold=15.0):
        '''
        Enhanced version of get_light_state that also returns distance and TTC (Time To Change) phase end ratio
        
        Args:
            vehicle: carla.Vehicle
            offset: float, offset distance ahead of vehicle
            dist_threshold: float, maximum distance threshold for traffic light detection
            
        Returns:
            tuple: (light_state, light_loc, light_id, distance, ttc_phase_end_ratio)
            - light_state: carla.TrafficLightState or None
            - light_loc: np.array([x, y, z]) or None (relative to vehicle)
            - light_id: int or None
            - distance: float or None (distance to traffic light in meters)
            - ttc_phase_end_ratio: float or None (ratio of time remaining in current phase, 0.0 to 1.0)
        '''
        vec_tra = vehicle.get_transform()
        veh_dir = vec_tra.get_forward_vector()
        ...
        for i in range(TrafficLightHandler.num_tl):
            ...
            current_distance = np.sqrt((tv_loc.x-hit_loc.x)**2 + (tv_loc.y-hit_loc.y)**2)
            ...
            for wp in TrafficLightHandler.list_stopline_wps[i]:
                ...
                if (same_road or same_road_1) and dot_ve_wp > 0:
                    # This light is affecting our lane
                    loc_in_ev = trans_utils.loc_global_to_ref(wp.transform.location, vec_tra)
                    light_loc = np.array([loc_in_ev.x, loc_in_ev.y, loc_in_ev.z], dtype=np.float32)
                    light_state = traffic_light.state
                    light_id = traffic_light.id
                    ...
                    # Get the elapsed time in current phase from CARLA
                    elapsed_time = traffic_light.get_elapsed_time()
                        
                    # Get the total time for current phase based on current state
                    if light_state == carla.TrafficLightState.Red:
                        total_phase_time = traffic_light.get_red_time()
                    elif light_state == carla.TrafficLightState.Green:
                        total_phase_time = traffic_light.get_green_time()
                    elif light_state == carla.TrafficLightState.Yellow:
                        total_phase_time = traffic_light.get_yellow_time()
                        
                    # Calculate TTC as time remaining in current phase
                    ttc_phase_end_ratio = max(0.0, total_phase_time - elapsed_time) / total_phase_time
                    
                    break

        return light_state, light_loc, light_id, distance, ttc_phase_end_ratio
